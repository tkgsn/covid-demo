...
class TestMap(unittest.TestCase):
    ...
    def test_find_nearest_coord(self):
        lower_left = self.x_y_equal_map._find_nearest_state([0,0])
        upper_right = self.x_y_equal_map._find_nearest_state([1,1])
        lower_right = self.x_y_equal_map._find_nearest_state([1,0])
        upper_left = self.x_y_equal_map._find_nearest_state([0,1])
        middle_left = self.x_y_equal_map._find_nearest_state([0,0.5])
        middle_right = self.x_y_equal_map._find_nearest_state([1,0.5])
    ...
    def test_state_to_location(self):
        location = [0,0]
